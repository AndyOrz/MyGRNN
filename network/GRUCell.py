...
class GRUCell_GAT(nn.Module):
    def __init__(self, input_size:int, hidden_size:int):
        ...
        self.ConvIR = dglnn.GATConv(input_size + hidden_size, hidden_size, 1, allow_zero_in_degree=True)
        self.ConvIZ = dglnn.GATConv(input_size + hidden_size, hidden_size, 1, allow_zero_in_degree=True)
        self.ConvIH = dglnn.GATConv(input_size + hidden_size, hidden_size, 1, allow_zero_in_degree=True)
    ...
    def forward(self, g, input, hx):
        ...
        r = torch.sigmoid(self.ConvIR(g, inputvalue)).squeeze(1) #重置门 => [N, num_heads, hidden_size] => [N, hidden_size]
        z = torch.sigmoid(self.ConvIZ(g, inputvalue)).squeeze(1) #更新门 => [N, num_heads, hidden_size] => [N, hidden_size]

        h = torch.tanh(self.ConvIH(g, torch.cat([input, r * hx], dim=1))).squeeze(1) #新记忆 => [N, num_heads, hidden_size] => [N, hidden_size]
